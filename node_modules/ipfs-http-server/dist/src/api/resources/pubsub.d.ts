export namespace subscribeResource {
    namespace options {
        namespace timeout {
            const socket: boolean;
        }
        namespace validate {
            export namespace options_1 {
                const allowUnknown: boolean;
                const stripUnknown: boolean;
            }
            export { options_1 as options };
            export const query: any;
        }
        const pre: {
            assign: string;
            /**
             * @param {import('../../types').Request} request
             * @param {import('@hapi/hapi').ResponseToolkit} _h
             */
            method: (request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, _h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>) => Promise<string>;
        }[];
    }
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
}
export namespace publishResource {
    export namespace options_2 {
        export namespace payload {
            const parse: boolean;
            const output: string;
        }
        const pre_1: ({
            assign: string;
            /**
             * @param {import('../../types').Request} request
             * @param {import('@hapi/hapi').ResponseToolkit} _h
             */
            method: (request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, _h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>) => Promise<string>;
        } | {
            assign: string;
            /**
             * @param {import('../../types').Request} request
             * @param {import('@hapi/hapi').ResponseToolkit} _h
             */
            method: (request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, _h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>) => Promise<Buffer>;
        })[];
        export { pre_1 as pre };
        export namespace validate_1 {
            export namespace options_3 {
                const allowUnknown_1: boolean;
                export { allowUnknown_1 as allowUnknown };
                const stripUnknown_1: boolean;
                export { stripUnknown_1 as stripUnknown };
            }
            export { options_3 as options };
            const query_1: any;
            export { query_1 as query };
        }
        export { validate_1 as validate };
    }
    export { options_2 as options };
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    export function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    export function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
}
export namespace lsResource {
    export namespace options_4 {
        export namespace validate_2 {
            export namespace options_5 {
                const allowUnknown_2: boolean;
                export { allowUnknown_2 as allowUnknown };
                const stripUnknown_2: boolean;
                export { stripUnknown_2 as stripUnknown };
            }
            export { options_5 as options };
            const query_2: any;
            export { query_2 as query };
        }
        export { validate_2 as validate };
    }
    export { options_4 as options };
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    export function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    export function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
}
export namespace peersResource {
    export namespace options_6 {
        export namespace validate_3 {
            export namespace options_7 {
                const allowUnknown_3: boolean;
                export { allowUnknown_3 as allowUnknown };
                const stripUnknown_3: boolean;
                export { stripUnknown_3 as stripUnknown };
            }
            export { options_7 as options };
            const query_3: any;
            export { query_3 as query };
        }
        export { validate_3 as validate };
        const pre_2: {
            assign: string;
            /**
             * @param {import('../../types').Request} request
             * @param {import('@hapi/hapi').ResponseToolkit} _h
             */
            method: (request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, _h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>) => Promise<string>;
        }[];
        export { pre_2 as pre };
    }
    export { options_6 as options };
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    export function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
    /**
     * @param {import('../../types').Request} request
     * @param {import('@hapi/hapi').ResponseToolkit} h
     */
    export function handler(request: import("@hapi/hapi").Request<import("@hapi/hapi").ReqRefDefaults>, h: import("@hapi/hapi").ResponseToolkit<import("@hapi/hapi").ReqRefDefaults>): Promise<import("@hapi/hapi").ResponseObject>;
}
export type Message = import('@libp2p/interface-pubsub').Message;
//# sourceMappingURL=pubsub.d.ts.map